...
(define (ok-string? value)
  (cond
    [(and (list? value) (empty? value)) #t]
    [(and (list? value ) (symbol? (car value))) (ok-string? (cdr value))]
    [else
      #f 
      ]))
...
(define (pick lst)
  (if (empty? lst) #f
      (getnum lst (random (length lst)))))

(define (getnum lst index)
  (cond
    [(empty? lst) #f]
    [(= index (- 1 (length lst))) (last lst)] 
    [else
     (list-ref lst index)]))
  ...
  (cond
    ...
    [(struct? exp)
     (cond
        ...
        [(either? exp) (if (flip .5) (generate-string-from-reg-exp (either-arg1 exp))
                           (generate-string-from-reg-exp (either-arg2 exp)))]
        [(repeat? exp) (if (flip .5) (append (generate-string-from-reg-exp exp) (generate-string-from-reg-exp (repeat-arg1 exp)))
                           '())])])  
  )
...
(define (dfa-help mach current str)
  (cond
    [(empty? str) current]
    [else (dfa-help mach (lookup (list current (car str)) (dfa-transitions mach)) (cdr str))])
  )

(define (dfa-accepts? mach str)
 (if (member (dfa-help mach (dfa-start-state mach) str) (dfa-accepting-states mach)) #t
     #f))
...
(define (list-leaf-labels tree)
  (cond
    ...
    [else
     (append (list-leaf-labels (car tree)) (list-leaf-labels (cdr tree)))])
  )
...
(define (lookupnew val lst [result '()])
    (cond
    [(empty? lst) result]
    [(equal? (rule-lhs (car lst)) val) (lookupnew val (cdr lst) (cons (rule-rhs (car lst)) result))]
    [else
     (lookupnew val (cdr lst) result)]
    )
  ) 

(define (generate-parse-tree-from-cfg grammar [key (cfg-start-symbol grammar)])
  (cond
    [(member key (cfg-terminals grammar)) (leaf key)]
    [else
     (node key (map (lambda (x) (generate-parse-tree-from-cfg grammar x)) (pick (lookupnew key (cfg-rules grammar)))))]
    )
  ;(gen (cfg-terminals grammar) (cfg-nonterminals grammar) (cfg-start-symbol grammar) (cfg-rules grammar))
  ) 

(define (gen term nonterm start rules)
  (cond
    [(member start term) (leaf start)]
    [else
     (node start (map (lambda (x) (gen term nonterm x rules)) (lookupnew start rules)))])
) 
...
(define exp-mcd
  (concat (repeat (concat (either '(it) (concat (either '(a) '(the)) (either (either '(mouse) '(cat)) '(dog))))
                          (concat (either '(dreamed) '(believed)) '(that))))
          (concat (either '(it) (concat (either '(a) '(the)) (either (either '(mouse) '(cat)) '(dog))))
                  (either (either '(slept) '(swam))
                          (concat (either '(evaded) '(chased)) (either '(it) (concat (either '(a) '(the)) (either (either '(mouse) '(cat)) '(dog))))))))                
  )
...
(define my-cfg
  (cfg
   '(i me you loved liked tolerated the song concert wedding school)
   ))
